...
public class AirportController {
    ...
    private final Map<Integer, List<Integer>> bookedTickets = new HashMap<>();
...
//        return 0;
    ...
    public int calculateFlightFare(@RequestParam("flightId")Integer flightId){
        ...
        int noOfPeopleWhoHaveAlreadyBooked = bookedTickets.getOrDefault(flightId, new ArrayList<>()).size();
...
//       return 0;
    ...
    public String bookATicket(@RequestParam("flightId")Integer flightId,@RequestParam("passengerId")Integer passengerId){
        ...
        if (flight == null || passenger == null || bookedTickets.getOrDefault(flightId, new ArrayList<>()).contains(passengerId)) {
            return "FAILURE";
        }

        List<Integer> passengersList = bookedTickets.getOrDefault(flightId, new ArrayList<>());
        if (passengersList.size() >= flight.getMaxCapacity()) {
            return "FAILURE";
        }

        passengersList.add(passengerId);
        bookedTickets.put(flightId, passengersList);
...
//        return null;
    ...
    public String cancelATicket(@RequestParam("flightId")Integer flightId,@RequestParam("passengerId")Integer passengerId){
        ...
        Flight flight = flights.get(flightId);
        if (flight == null || !bookedTickets.containsKey(flightId) || !bookedTickets.get(flightId).contains(passengerId)) {
            return "FAILURE";
        }

        List<Integer> passengersList = bookedTickets.get(flightId);
